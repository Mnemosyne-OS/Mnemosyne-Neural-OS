...
Note that `openclaw` is unlike the others: OpenClaw 2 keeps live sessions in SQLite
...
a bridge in a sandboxed frame, or `node:fs` on a server. The `/node` entry adds the
...
- It reads what a harness already wrote. If a harness records no working
  directory, that column is empty, and it is never filled by guessing.
...
| **`@mnemosyne_os/agent-transcripts`** *(you are here)* | Read what **coding agents already write on disk**: the connector format and the interpreter |
